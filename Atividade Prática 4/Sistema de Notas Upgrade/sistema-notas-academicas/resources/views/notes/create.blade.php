...
@section('title', 'Nova Nota - Sistema de Notas Acadêmicas')

@section('header')
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Criar Nova Nota</h2>
@endsection
                ...
                <form action="{{ route('notes.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="conteudo" id="conteudo" rows="12" required
                                  ...
                                  class="block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent">{{ old('conteudo') }}</textarea>
                        @error('conteudo')
                            <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                        @enderror
                        <p class="text-xs text-gray-500 mt-2 flex items-center">
                            ...
                            O conteúdo será criptografado e armazenado com segurança máxima
                        </p>
                    </div>
